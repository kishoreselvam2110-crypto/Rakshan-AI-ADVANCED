...
fastify.post('/api/smart-trip', async (request, reply) => {
  ...
  // Check MySQL Persistent Cache
try {
    const [rows] = await pool.query('SELECT * FROM itineraries WHERE cache_key = ? LIMIT 1', [cacheKey]);
    
    if (rows.length > 0) {
        const dbTrip = rows[0];
        // Parse the data if it's stored as a JSON string in MySQL
        const tripData = typeof dbTrip.data === 'string' ? JSON.parse(dbTrip.data) : dbTrip.data;
        
        await setCachedTrip(cacheKey, tripData);
        return { success: true, ...tripData };
    }
} catch (error) {
    console.error("Database cache error:", error);
    // Let it fall through to fetch a new trip plan if the cache check fails
}
  }
