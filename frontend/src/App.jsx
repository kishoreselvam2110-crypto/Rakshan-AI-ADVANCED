    ...
    <AnimatePresence mode="wait">
      <Suspense fallback={<div className="h-screen flex items-center justify-center bg-[#050505]"><div className="w-8 h-8 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin" /></div>}>
        <Routes location={location} key={location.pathname}>
          ...
          <Route path="/scam-check" element={<PageWrapper><ScamCheck /></PageWrapper>} />
          <Route path="/connectivity" element={<PageWrapper><ConnectivityMonitor /></PageWrapper>} />
          <Route path="/lost-item" element={<PageWrapper><LostItemAssistant /></PageWrapper>} />
          <Route path="/safety-center" element={<PageWrapper><SafetyCenter /></PageWrapper>} />
          <Route path="/offline-kit" element={<PageWrapper><OfflineKit /></PageWrapper>} />
        </Routes>
      </Suspense>
    </AnimatePresence>
  );
...
function Navbar() {
  ...
  const triggerSOS = async () => {
    ...
    const sendSos = async (lat, lon, isSimulated = false) => {
      ...
      const payload = {
        ...
        name: userData?.name || "Live Tourist",
        location: { lat, lon },
        timestamp: new Date().toISOString(),
        message: isSimulated 
          ? "🚨 SIMULATED SOS: Browser GPS restricted. Transmitting base coordinates."
          : "🚨 EMERGENCY SOS: Immediate assistance required!"
      };

      if (!navigator.onLine) {
        const { queueOfflineSOS } = await import("./utils/offlineSync");
        await queueOfflineSOS(payload);
        toast.warning("Network Offline: Emergency SOS Queued in Local Vault. Will auto-retry once connected.", { duration: 6000 });
        setSosLoading(false);
        return;
      }
      ...
      try {
        ...
        toast.success(isSimulated ? "Simulated Signal Transmitted & Acknowledged!" : "Rakshan SOS Acknowledged by Command Center!", { icon: '🛡️', duration: 5000 });
      } catch (e) {
        console.error("SOS Transmission Error:", e);
        const { queueOfflineSOS } = await import("./utils/offlineSync");
        await queueOfflineSOS(payload);
        toast.error("Network Error: Signal queued locally for auto-retry.");
      } finally {
        setSosLoading(false);
      }
    };
    ...
    if (useSimulated) {
      return sendSos(13.0827, 80.2707, true);
    }

    navigator.geolocation.getCurrentPosition(
      (pos) => sendSos(pos.coords.latitude, pos.coords.longitude),
      (err) => {
        console.error("SOS Geolocation Error, auto-transmitting simulated signal for demo:", err);
        sendSos(13.0827, 80.2707, true);
      },
  ...
  const navLinks = [
    ...
    { to: "/scam-check", label: "Scam Check", icon: <ShieldAlert size={18} /> },
    { to: "/lost-item", label: "Lost Item", icon: <ShieldAlert size={18} /> },
    { to: "/admin", label: "Command Center", icon: <AlertTriangle size={18} />, color: "text-indigo-400" },
  ];
    ...
    <nav className="sticky top-0 z-[60] bg-black/40 backdrop-blur-2xl border-b border-white/5" role="navigation" aria-label="Main Navigation">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <Link to="/home" className="flex items-center gap-3 group focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-lg p-1" aria-label="Rakshan AI Home">
            <span className="text-2xl font-black tracking-tighter text-white group-hover:text-indigo-400 transition-colors">
              Rakshan<span className="text-indigo-500">AI</span>
            </span>
      ...
      <Router>
        <GsapGlitchWrapper containerRef={containerRef}>
          ...
          <footer className="text-center py-10 text-white/20 text-[10px] uppercase tracking-[0.2em] font-bold border-t border-white/5" role="contentinfo">
            &copy; 2026 Rakshan AI • National Tourist Safety Protocol
          </footer>
        </div>
        </GsapGlitchWrapper>
      </Router>
